...
public class RF {
	...
	private static void performCalibration(Instances ninetyPercentData, Instances tenPercentData, int numRepetitions, String classifier, RandomForest classifierObject, int folds, String size) throws Exception {
		...
		for (int i = 0; i < numRepetitions; i++) {
			...
			// seperate the RandomForest prediction scores and the labels
			...
			// RandomForest prediction scores and label from the first 5%
...
//			System.out.println("AUC after calibration: " + AUCAfterCalibration);
	...
	public static void runModels(int folds, String size, Instances trainingData, Instances testingData, String classifier, RandomForest classifierObject) throws Exception {
		System.out.println("Start Training Model");
		long startTrainingTime = System.currentTimeMillis();
		classifierObject.buildClassifier(trainingData);
		...
		testingPhaseEval.evaluateModel(classifierObject, testingData);
	...
	public static void main(String[] args) throws Exception {
		...
		int NUMATTRIBUTES = data.instance(0).numAttributes() - 1;
		RandomForest classifierObject = new RandomForest();
		...
		if (size.equals("sparse")) {
			classifierObject.setOptions(Utils.splitOptions("-P 50 -I 100 -num-slots 1 -K " + (int)Math.cbrt(NUMATTRIBUTES) + " -M 1.0 -V 0.001 -S 1"));
			System.out.println("Set hyper-parameters to size sparse");
		} else if (size.equals("dense")) {
			classifierObject.setOptions(Utils.splitOptions("-P 50 -I 175 -num-slots 1 -K " + (int)Math.sqrt(NUMATTRIBUTES) + " -M 1.0 -V 0.001 -S 1"));
