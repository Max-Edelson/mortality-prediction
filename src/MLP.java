...
public class MLP {
	...
	private static void performCalibration(Instances ninetyPercentData, Instances tenPercentData, int numRepetitions, String classifier, MultilayerPerceptron mlp, int folds, String size) throws Exception {
		...
		System.out.println();
		mlp.buildClassifier(ninetyPercentData);
		...
		for (int i = 0; i < numRepetitions; i++) {
			...
			// seperate the MultilayerPerceptron prediction scores and the labels
			...
			for (int j = 0; j < trainingData.numInstances(); j++) {
				...
				double[] instanceDistribution = mlp.distributionForInstance(trainingData.get(j));
			...
			for (int j = 0; j < testingData.numInstances(); j++) {
				...
				double[] instanceDistribution = mlp.distributionForInstance(testingData.get(j));
			...
			// MultilayerPerceptron prediction scores and label from the first 5%
			...
			calibrationTestingEval.evaluateModel(mlp, testingData);
			int classIndex = 0;
...
//			System.out.println("AUC after calibration: " + AUCAfterCalibration);
	...
	public static void runModels(int folds, String size, Instances trainingData, Instances testingData, String classifier, MultilayerPerceptron mlp) throws Exception {
		...
		testingPhaseEval.evaluateModel(mlp, testingData);
	...
	public static void main(String[] args) throws Exception {
		...
		MultilayerPerceptron classifierObject = new MultilayerPerceptron();
		String classifierName = "Multilayer Perceptron";
