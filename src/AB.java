...
public class AB {
	...
	private static void performCalibration(Instances ninetyPercentData, Instances tenPercentData, int numRepetitions, String classifier, AdaBoostM1 ada, int folds, String size) throws Exception {
		...
		System.out.println();
		ada.buildClassifier(ninetyPercentData);
		...
		for (int i = 0; i < numRepetitions; i++) {
			...
			// seperate the adaboost prediction scores and the labels
			...
			for (int j = 0; j < trainingData.numInstances(); j++) {
				...
				double[] instanceDistribution = ada.distributionForInstance(trainingData.get(j));
			...
			for (int j = 0; j < testingData.numInstances(); j++) {
				...
				double[] instanceDistribution = ada.distributionForInstance(testingData.get(j));
			...
			// adaboost prediction scores and label from the first 5%
			...
			calibrationTestingEval.evaluateModel(ada, testingData);
			int classIndex = 0;
...
//			System.out.println("AUC after calibration: " + AUCAfterCalibration);
	...
	public static void runModels(int folds, String size, Instances trainingData, Instances testingData, String classifier, AdaBoostM1 ada) throws Exception {
		System.out.println("Start Training Model");
		long startTrainingTime = System.currentTimeMillis();
		ada.buildClassifier(trainingData);
		...
		testingPhaseEval.evaluateModel(ada, testingData);
	...
	public static void main(String[] args) throws Exception {
        ...
        Instances trainingData = new Instances(data, 0, trainSize);			// 90%
        Instances testingData = new Instances(data, trainSize, testSize);	// 10%
		...
		String classifierName = "AdaBoost";
		...
		if (size.equals("sparse")) {
			ada.setOptions(Utils.splitOptions("-P 100 -S " + seed + " -I 70 -Q -W weka.classifiers.trees.J48 -- -C 0.25 -M 2"));
			System.out.println("Set hyper-parameters to size sparse");
		} else if (size.equals("dense")) {
			ada.setOptions(Utils.splitOptions("-P 100 -S " + seed + " -I 20 -Q -W weka.classifiers.trees.J48 -- -C 0.25 -M 2"));
		...
		runModels(folds, size, trainingData, testingData, classifierName, ada);
		performCalibration(trainingData, testingData, numRepetitions, classifierName, ada, folds, size);
	}
}
